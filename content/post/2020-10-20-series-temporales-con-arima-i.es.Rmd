...
Ahora que mi profe soy yo, me he decidido a entender estos modelos clásicos para ver si les saco partido para el pronóstico de series temporales. Me han sorprendido gratamente son modelos robustos y poderosos,  funcionan muy bien, por lo que me vamos a dedicarles al menos 2 artículos, empecemos:
...
Las series temporales *estacionarias* son habituales del mundo físico, pero no así en ciencias económicas o sociales donde suele haber tendencias y variaciones no homogeneas en el tiempo.

Los modelos arima son muy utilizados porque como veremos, se puede transformar cualquier serie *no estacionaria* y convertirla en *estacionaria* con transformaciones matemáticas "sencillas".

## ACF y PACF 
Para saber cómo aplicar los modelos *ARIMA* tendremos que aprender a interpretar las gráficas de autocorrelación *ACF* y autocorrelacion parcial *PACF* de una serie temporal. Estas gráficas nos ayundan a estimar los órdenes (recuerda..los 3 parámetros) `(p,d,q)` del modelo *ARIMA*.

En $R$ tenemos mil formas de usar *ARIMA*, pero empezaremos con las librerías `astsa` y `forecast` que son muy completas y tiene muchas funciones útiles. La librería forecast es imprescindible para pronostico de series temporales pues tiene otros tantos modelos en ella.

Aunque existe de la función de RBase `acf()` para representar las gráficas de autocorrelación, prefiero pintarlas con cualquiera de estas otras funciones :

 - `acf2()` del paquete `astsa` 
 - `ggtsdisplay()` o `tsdisplay()` del paquete `forecast` en plot o ggplot .

Veamos un ejemplo:

```{r acf_graficas, warning=FALSE,message=FALSE}
# Gráficas ACF y PACF de la serie Nile
library(forecast)
...
acf2(diff(Nile))
```
En cualquiera de estas representaciones, la banda entre las líneas azules intermitentes indican la zona de valores **NO SIGNIFICATIVOS** de correlación, que a efectos prácticos equivale a **cero** correlación.
...
Luego con de estas dos columnas se calcula el coeficiente de correlación entre ambas y pintamos ese valor en la gráfica, esto sería para `x lag=1`. Si corremos la columna 2 un retraso más y volvemos a calcular el coef.correlación entre $X_t$ (col1) y  $X_{t-2}$ (*hoy vs. anteayer*). Esto lo podemos hacer `n` veces, hasta que llegamos al último intervalo de la serie temporal, aunque cada vuelta tenemos una fila menos y por tanto es menos representativa, por eso siempre interpretamos los primeros lag, que son los importantes.

Los valores de ACF van de -1 a 1, Un valor próximo a 1 indica una gran correlación entre las columnas, si es próximo a -1 la correlación es inversa (los valores de hoy suben cuando los de ayer bajaron, por ejemplo), y uno próximo a 0 significa que las columnas comparadas son  independientes = no podemos predecirlos (nada nos dice el valor de ayer respecto al que tenemos hoy).

La gráfica `PACF` es la `ACF` de la `ACF`.

Pues resulta que estas gráficas nos pueden dar los órdenes para ajustar un modelo *ARIMA*, la clave está en interpretarlas según la siguiente tabla:

|    | AR(p) | MA(q)	 | ARMA(p,q) |
| -- | -- | -- | -- |
|ACF	| varios puntos con coef>0 | 0 excepto los `q` primeros |	varios puntos con coef>0  | 
...
La manera más simple de transformar la serie a estacionaria es **diferenciandola**, que consiste simplemente en restar al valor de hoy, el de ayer, y así quedarnos con una serie temporal nueva llamada de diferencias, en $R$ se hace con la función `diff(x)`.
...
Según la nomeclatura de los modelos `ARIMA(p,d,q)`, el número de integraciones o diferenciaciones que hay que hacer a una serie para convertirla en estacionaria se da con el parámetro `d`.
...
El proceso que sigo es:
 ...
 1. Pintar la serie temporal ($X_t$) e identificar valores inusuales (outliers). En tal caso uso `forecast::tsclean(x)`.
 ...
 2. Si en la gráfica apreciamos que la varianza no es homogenea, hay que usar algún método de  transformarción de varianza como tomar `log(x)` o `Box-Cox(x)` calculando `lambda`.
 2. Examinar las ACF/PACF y proponer modelos `ARIMA(p,d,q)` con la interpretación dada en la tabla. Alternativamente usar `auto.arima()` para hallar el mejor modelo de ajuste automático (que no siempre es mejor).
 ...
 2. Chequear los graficos de residuos del modelo, por ejemplo con la función `sarima()` para comprobar que sea ruido blanco, pues en caso contrario todavía hay información relevante en ellos y el modelo no es completo ---> añadir algún órden.
